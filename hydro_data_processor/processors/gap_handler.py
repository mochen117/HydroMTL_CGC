...
class GapHandler:
    ...
    def handle_missing_data(self, df: pd.DataFrame, 
                           variable_types: Dict[str, List[str]] = None) -> pd.DataFrame:
        """
        ...
        for gap_start, gap_end in gap_segments:
            ...
            if gap_length <= max_gap or max_gap == 0:
                ...
                filled_values = self._fill_gap(
                    series, gap_start, gap_end, strategy.get('method', 'linear')
                )
    ...
    def _find_gap_segments(self, is_missing: pd.Series) -> List[Tuple[int, int]]:
        """
    ...
    def _fill_gap(self, series: pd.Series, 
        ...
        valid_before = pre_gap_idx >= 0 and not pd.isna(series.iloc[pre_gap_idx])
        valid_after = post_gap_idx < len(series) and not pd.isna(series.iloc[post_gap_idx])
        ...
        if method == 'linear':
            ...
            return pd.Series(filled_values, index=range(gap_start, gap_end + 1))
            ...
            try:
                ...
                if len(window_values) >= 4:  # Need at least 4 points for cubic
                    spline = interpolate.CubicSpline(window_indices, window_values)
        ...
        for column in df.columns:
            if pd.api.types.is_numeric_dtype(df[column]):
                ...
                if missing_pct > 0:
                    ...
                    if gap_segments:
                        max_gap = max(end - start + 1 for start, end in gap_segments)
        ...
        for col in numeric_cols:
            if col in original_df.columns and col in processed_df.columns:
                ...
                if original_missing > processed_missing:
                    ...
                    logger.info(f"Gap handling for '{col}': "

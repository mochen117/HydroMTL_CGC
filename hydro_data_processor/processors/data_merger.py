...
class DataMerger:
    ...
    def merge_basin_data(self, 
        ...
        merged_df = self._merge_data_source(base_df, streamflow_data, 'streamflow')
        ...
        logger.info(f"Merged data with {len(merged_df)} days and {len(merged_df.columns)} columns")
        return merged_df
    ...
    def _merge_et_data(self, base_df: pd.DataFrame, et_data: pd.DataFrame) -> pd.DataFrame:
    ...
    def _merge_smap_data(self, base_df: pd.DataFrame, smap_data: pd.DataFrame) -> pd.DataFrame:
        """Merge SMAP soil moisture data."""
        # Process SMAP 3-day pattern
        smap_processed = self.time_processor.process_smap_3day_pattern(smap_data)
    ...
    def check_data_completeness(self, merged_df: pd.DataFrame) -> Dict[str, Any]:
        """

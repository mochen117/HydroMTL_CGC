...
class QualityChecker:
    ...
    def check_dataset_quality(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if results['failed_checks'] > 0:
            ...
            critical_issues = ['Streamflow coverage below threshold',
                               'Missing date column',
                               'No streamflow data']
            if any(issue in str(results['issues'])
    ...
    def _check_basic_structure(
            self, df: pd.DataFrame, results: Dict[str, Any]) -> bool:
    ...
    def _check_streamflow_quality(
        ...
        if not valid_values.empty:
            if (valid_values < valid_min).any() or (
                    valid_values > valid_max).any():
                warning = f'Streamflow values outside expected range: [{valid_min}, {valid_max}]'
                results['warnings'].append(warning)

    def _check_forcing_quality(
            self, df: pd.DataFrame, results: Dict[str, Any]):
        ...
        if 'tmax' in df.columns and 'tmin' in df.columns:
            ...
            if not tmax_valid.empty and (
                (tmax_valid < temp_min) | (
                    tmax_valid > temp_max)).any():
                warning = f'Temperature values outside expected range: [{temp_min}, {temp_max}]°C'
                results['warnings'].append(warning)

            if not tmin_valid.empty and (
                (tmin_valid < temp_min) | (
                    tmin_valid > temp_max)).any():
                warning = f'Temperature values outside expected range: [{temp_min}, {temp_max}]°C'
                results['warnings'].append(warning)
        ...
        if 'prcp' in df.columns:
            ...
            if not prcp_valid.empty and (
                (prcp_valid < prcp_min) | (
                    prcp_valid > prcp_max)).any():
    ...
    def generate_quality_report(self, quality_results: Dict[str, Any]) -> str:
        ...
        if quality_results['warnings']:
            ...
            # Limit to first 10
            for warning in quality_results['warnings'][:10]:
                report_lines.append(f"  - {warning}")
            if len(quality_results['warnings']) > 10:
                report_lines.append(
                    f"  ... and {len(quality_results['warnings']) - 10} more warnings")

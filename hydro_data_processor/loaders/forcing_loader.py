...
class ForcingLoader:
    ...
    def load(self, gage_ids: List[str], **kwargs) -> Optional[pd.DataFrame]:
        ...
        for gage_id in gage_ids:
            try:
                ...
                logger.warning(
                    f"Failed to load forcing for gage {gage_id}: {e}")
    ...
    def _load_single_gage(self, gage_id: str, huc2: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            logger.error(
                f"Error reading forcing file for gage {gage_id}: {e}")
            return None
        ...
        for huc_dir in base_dir.iterdir():
            if huc_dir.is_dir() and huc_dir.name.isdigit():
                ...
                if possible_file.exists():
                    logger.debug(
                        f"Found gage {gage_id} in HUC2 {huc2_name}")
                    return huc2_name
    ...
    def _read_forcing_file(self, file_path: Path, gage_id: str) -> pd.DataFrame:
        ...
        try:
            ...
            if has_header:
                ...
                # If gage_id column exists, drop it (we'll add it back later)
                ...
                if num_cols == 11:
                    ...
                    # Format with gage_id: gage_id year month day + 8 forcing variables
                    forcing_vars = ['SWdown', 'LWdown', 'Tair', 'Qair', 'Wind', 'Rainf', 'Snowf', 'Psurf']

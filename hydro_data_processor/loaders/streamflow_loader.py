"""
Streamflow loader.
Handles parsing of CAMELS and USGS format streamflow files.
Automatically converts negative erroneous values to NaN.
"""
    ...
    def _read_streamflow_file(self, file_path: Path, gage_id: str) -> pd.DataFrame:
        ...
        if is_usgs_data:
            try:
                ...
                has_header = ('GAGE_ID' in first_line.upper() or 'gage_id' in first_line.lower() or 'Year' in first_line)
                ...
                if has_header and is_comma_separated:
                    ...
                    for col in df.columns:
                        ...
                        elif 'year' in col_lower: column_mapping[col] = 'year'
                        elif 'month' in col_lower or 'mnth' in col_lower: column_mapping[col] = 'month'
                        elif 'day' in col_lower: column_mapping[col] = 'day'
                        elif 'streamflow' in col_lower: column_mapping[col] = 'streamflow'
                        elif 'qc' in col_lower or 'flag' in col_lower: column_mapping[col] = 'qc_flag'
                ...
                logger.error(f"Failed to read USGS file: {e}")
                raise
        else:
            try:
                ...
                if not all(col in df.columns for col in expected_cols):
                    df = pd.read_csv(file_path, sep=r'\s+', header=None, dtype=str)
                    if df.shape[1] == 6: df.columns = expected_cols
                    elif df.shape[1] == 5:
                        df.columns = ['gage_id', 'year', 'month', 'day', 'streamflow']
                        df['qc_flag'] = None
            except Exception as e:
                df = pd.read_csv(file_path, sep=r'\s+', header=None, dtype=str)
                if df.shape[1] == 6: df.columns = ['gage_id', 'year', 'month', 'day', 'streamflow', 'qc_flag']
                elif df.shape[1] == 5:
                    df.columns = ['gage_id', 'year', 'month', 'day', 'streamflow']
                    df['qc_flag'] = None
        ...
        df.loc[df['streamflow'] < -900.0, 'streamflow'] = np.nan
        ...
        for huc_dir in self.data_source_path.iterdir():
            if huc_dir.is_dir() and huc_dir.name.isdigit():
                for file in huc_dir.glob("*_streamflow_qc.txt"):
                    gages.append(file.name.split('_')[0])

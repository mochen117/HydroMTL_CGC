"""
SMAP loader for CAMELS data.
Loads soil moisture data.
"""
...
class SMAPLoader(BaseDataLoader):
        ...
        for gauge_id in gauge_ids:
            try:
                ...
                if gauge_data is not None and not gauge_data.empty:
                    ...
                    logger.debug(f"Loaded SMAP data for gauge {gauge_id}")
                else:
                    logger.warning(f"No SMAP data found for gauge {gauge_id}")
            except Exception as e:
                logger.error(
                    f"Error loading SMAP data for gauge {gauge_id}: {e}")
    ...
    def _load_single_gauge(self, gauge_id: str, **
        ...
        """Load SMAP data for a single gauge."""
        ...
        if not file_path or not file_path.exists():
            ...
            if not file_path or not file_path.exists():
                logger.debug(
                    f"SMAP file not found for gauge {gauge_id} in HUC2 {huc2}")
                return None
    ...
    def _find_alternative_path(
        ...
        alt_path = self.config.data_source_path / "NASA_USDA_SMAP_CAMELS" / \
            huc2 / f"{gauge_id}_lump_nasa_usda_smap.txt"
        if alt_path.exists():
            return alt_path

# ==============================================================================
# Description:
#   Run Chapter 4 STL-ET climate-consistency controlled experiments.
#
# Purpose:
#   Provide single-task evapotranspiration baselines for evaluating CGC
#   evapotranspiration performance under different train-test climate
#   consistency groups.
# ==============================================================================
...
def build_config(group_name: str, basin_list_path: Path) -> Path:
    """Build one temporary STL-ET config for one climate-consistency group."""
    cfg = load_yaml(BASE_CONFIG)
    cfg = apply_stlet_config(cfg)

    run_name = f"ch4_consistency_{group_name}_stlet_seed42"
...
def main() -> None:
    """Run all STL-ET climate-consistency experiments."""
    ...
    for group_name, basin_list_path in GROUP_FILES.items():
        if not basin_list_path.exists():
            raise FileNotFoundError(
                ...
                "Run scripts/ch4/build_climate_consistency_groups.py first."
            )
    ...
    print("All Chapter 4 STL-ET climate-consistency experiments completed.")
